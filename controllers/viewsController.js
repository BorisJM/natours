...
exports.getTour = catchAsync(async (req, res, next) => {
  ...
  let booking;
  if (req.user)
    booking = await Booking.findOne({ user: req.user.id, tour: tour._id });
  if (req.user)
    review = await Review.findOne({ user: req.user.id, tour: tour._id });

  if (booking) req.user.hasTour = true;
  if (review) req.user.reviewTour = slug;
  // if (!tour) {
...
exports.getLikedTours = catchAsync(async (req, res) => {
  const tours = await Tour.find({
    _id: { $in: req.user.favouriteTours },
  });
...
exports.getMyReviews = catchAsync(async (req, res) => {
  ...
  const tours = await Promise.all(
    reviews.map(async (rev) => await Tour.findById(rev.tour)),
  );
...
exports.manageTours = catchAsync(async (req, res) => {
  ...
  res.status(200).render('managePage', {
    title: 'Manage tours',
    users,
    tours,
    countries,
